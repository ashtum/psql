...
asio::awaitable<void> run_exmaple(psql::connection& conn)
{
  ...
  co_await conn.async_query("INSERT INTO actors VALUES ($1, $2);", std::tuple{ "Bruce Lee", 32 }, asio::deferred);
  co_await conn.async_query("INSERT INTO actors VALUES ($1, $2);", std::tuple{ "Jackie Chan", 70 }, asio::deferred);
  ...
  // Example 3
  auto result =
    co_await conn.async_query("SELECT $1 as array;", std::tuple{ std::vector{ "1", "2", "3" } }, asio::deferred);
